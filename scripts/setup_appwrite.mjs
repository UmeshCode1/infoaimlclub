...
async function setup() {
    console.log('🚀 Starting Appwrite Infrastructure Check...');

    try {
        // ... (Teams and Database creation same as before)
        ...
        const collections = [
            ...
            // ... (adding other collections as needed, but focusing on fixing COLL_LINKS first)
        ];
            ...
            for (const attr of col.attributes) {
                if (!existingKeys.includes(attr.key)) {
                    ...
                    if (attr.type === 'string') {
                        ...
                        await databases.createBooleanAttribute(DB_ID, col.id, attr.key, attr.required, attr.active ?? true);
                    }
                    // Wait a bit for each attribute
